...
2. **无对应**：对不上任何一行，或命中下方否决条件 → **不要选 pack**，走通用 UI（路径 B：`library/examples/generic/` + 硬规则 + 自拟 token）。
3. 矩阵也可在通用路径下作「同类游戏怎么分区」的参考，但**不绑定**该行的色板 / 几何 / 动效。

## 强制改判「通用」的否决条件（命中任一条 → 路径 B）

1. **玩法循环对不上**：用户核心循环与候选行「核心玩法循环」列无法逐项对应（仅气质像、品类名像不够）。
2. **楼型冲突**：用户需要的主布局与该行「布局原型」冲突（例：要 JRPG 菜单栈却选街机标题分；要能量台却选魂类极简底栏）。
3. **多行均沾边但无一主导**：没有一行在「类型 + 玩法」上同时领先 → 通用，勿平均凑合。
4. **用户只要中性/品牌自研视觉**：明确不要本库 10 套专有视觉语汇 → 通用。
5. **会引入禁止模板**：套用该 pack 会把统一锁定开火条塞进矩阵不允许的类型 → 通用或另选真正匹配行。
