...
**本项目为移动端竖屏服务**（默认画幅 9:20，主输入为触控）。桌面浏览器仅作预览壳。通用 HUD 思路（分层、图标主键、中文、避网页感）见 `research/synthesis/mobile-portrait-first.md`。

## 核心原则

**建材共享,楼型按游戏类型分；交付形态是竖屏触控。**
...
- **竖屏优先**：场景默认 9:20 手机框；操作必须有可见触控控件；禁止以键鼠快捷键为唯一操作路径
- **触控语义**：提示文案用点按/拖动/长按，不用 LMB/RMB/WASD 作为主说明
- **常驻键用图标/物件**：主键面不排汉字胶囊；中文留给情境提示、菜单与 `aria-label`（见 `mobile-portrait-first.md`）
...
- Demo 默认 **移动端竖屏 + 触控**；键鼠仅调试捷径，不得作为玩家主路径
